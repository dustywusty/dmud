...
func (ss *SpawnSystem) processSpawn(w *ecs.World, spawnEntity ecs.Entity) {
    spawn, err := util.GetTypedComponent[*components.Spawn](w, spawnEntity.ID, "Spawn")
    if err != nil {
        log.Error().Err(err).Msg("Error getting spawn component")
        return
    }

    room, err := util.GetTypedComponent[*components.Room](w, spawnEntity.ID, "Room")
    if err != nil {
